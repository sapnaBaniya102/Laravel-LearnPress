													...
													<div class="card-body">
														...
														@foreach ($coursecat as $coursecats)
																	...
																	<tr>
																		<td>{{ $loop->index+1 }}</td>
																		<td>
                                                                            <a href="{{ route('coursecat.edit', $coursecats->id) }}"><button class="p-2 rounded btn-primary">Edit</button></a>
																		<form action="{{ route('coursecat.destroy', $coursecats->id) }}" method="post">
                                                                        @csrf
                                                                        @method('DELETE')
																		...
																		<td>{{ $coursecats->name }}</td>
																		<td>{{ $coursecats->status }}</td>
